...
class GridWorld():
    ...
    def __init__(self, env_size=args.env_size,
        ...
        # define the parameters of an environment
        ...
        self.reward_transition_matrix = None
    ...
    def reset(self):
        # set the previous trajectory, if there exists.
        self.agent_state = self.start_state
        self.traj = [self.agent_state]
        return self.agent_state, {}

    def step(self, action):
        assert action in self.action_space, "Invalid action"  # raise error

        next_state, reward = self._get_next_state_and_reward(self.agent_state,
                                                             action)  # generate the next state, when s and a given
    ...
    def _get_next_state_and_reward(self, state, action):
        x, y = state
        new_state = tuple(np.array(state) + np.array(action))  # deterministic policy
        # modify the state by clipping
        if y + 1 > self.env_size[1] - 1 and action == (0, 1):  # down
            ...
            x = 0
            reward = self.reward_forbidden
        elif new_state == self.target_state:  # stay, continuing task
            x, y = self.target_state  # actually 'x,y = new_state'
    ...
    def render(self, animation_interval=args.animation_interval):
        if self.canvas is None:
            ...
            self.target_rect = patches.Rectangle((self.target_state[0] - 0.5, self.target_state[1] - 0.5), 1, 1,
    ...
    def det_transition_matrix(self):
        '''
        return state transition matrix, and reward transition matrix, as this environment is deterministic
        :return:
        '''
        stm = np.zeros((self.num_states,
                        len(self.action_space)))  # the latent dimension is 'num_states-1', as one movement actually can jump to any state except itself
        rtm = np.zeros((self.num_states,
                        len(self.action_space)))  # the latent dimension is 3, as there are three possible reward in the grid example
        for i in range(self.num_states):
            x = i % self.env_size[0]
            y = i // self.env_size[0]
            # see if the movement hits the wall
            for j, action in enumerate(self.action_space):
                next_state, reward = self._get_next_state_and_reward((x, y), action)
                stm[i, j] = next_state[1] * self.env_size[0] + next_state[0]
                rtm[i, j] = reward
        self.state_transition_matrix = stm
        self.reward_transition_matrix = rtm
